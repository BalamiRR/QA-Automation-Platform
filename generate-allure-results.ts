...
// First run the tests to generate the JSON report
try {
  console.log('Running cucumber tests...');
  execSync('npx cucumber-js tests/features --format json:reports/allure-results/allure-results.json', { stdio: 'inherit' });
} catch (error) {
...
if (fs.existsSync(cucumberReportPath)) {
  ...
  fs.readdirSync(allureResultsPath).forEach((file: string) => {
